...
function EmptyState({ onCreateClick }: { onCreateClick: () => void }) {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="glass rounded-2xl px-6 py-10 text-center">
      ...
      <motion.h3 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="text-lg font-semibold text-ink">¡Ahorrá para lo que importa!</motion.h3>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="mt-2 text-sm text-ink-muted max-w-md mx-auto">
        Las metas te ayudan a definir objetivos de ahorro, seguir tu progreso y mantenerte motivado. 
      </motion.p>
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.1 } } }} className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-lg mx-auto text-left">
        {[
          { icon: '🎯', title: 'Definí tu objetivo', desc: 'Elegí cuánto querés ahorrar y para cuándo. Podés dividirlo en checklist de gastos.' },
          { icon: '👥', title: 'Invitá a tu grupo', desc: 'Compartí la meta con amigos o familia. Cada uno aporta su cuota mensual.' },
          { icon: '📊', title: 'Seguí el progreso', desc: 'Visualizá cuánto llevás ahorrado y cuánto falta con gráficos y metas parciales.' },
        ].map((item, i) => (
          ...
          {viewMode === 'cards' ? (
            <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.05 } } }} className="space-y-3">
              {filteredGoals.map((goal) => (
                <motion.div key={goal.id} variants={{ hidden: { opacity: 0, y: 12 }, visible: { opacity: 1, y: 0 } }} whileHover={{ y: -1 }}>
                  <GoalCard meta={goal} />
                </motion.div>
              ))}
            </motion.div>
