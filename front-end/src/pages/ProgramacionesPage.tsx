  ...
  return (
    <div className="rounded-2xl border-2 border-dashed border-border bg-white px-6 py-16 text-center animate-fade-in">
      ...
      <button
        ...
        className="mt-6 inline-flex items-center gap-2 rounded-lg bg-accent px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-accent/90 focus:outline-none focus:ring-2 focus:ring-accent/30"
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div>
            ...
            <div className="flex rounded-lg border border-border overflow-hidden">
              <button type="button" onClick={() => setTipo('fijo')} className={`flex-1 px-3 py-2 text-[11px] font-semibold transition-colors ${tipo === 'fijo' ? 'bg-accent-subtle text-accent' : 'bg-white text-ink-muted hover:bg-surface-raised'}`}>
                Monto fijo
              </button>
              <button type="button" onClick={() => setTipo('porcentaje')} className={`flex-1 px-3 py-2 text-[11px] font-semibold transition-colors ${tipo === 'porcentaje' ? 'bg-accent-subtle text-accent' : 'bg-white text-ink-muted hover:bg-surface-raised'}`}>
                Porcentaje
              </button>
            </div>
          </div>
      ...
      {isLoading && (
        <div className="space-y-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-border bg-white px-5 py-4 space-y-3">
        ...
        <div className="space-y-3 stagger">
          {programaciones.map((prog) => {
            ...
            return (
              <div
                key={prog.id}
                className="rounded-2xl border border-border bg-white border-l-4 card-hover overflow-hidden"
