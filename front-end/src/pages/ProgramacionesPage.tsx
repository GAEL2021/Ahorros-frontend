...
function EmptyState({ onCreateClick }: { onCreateClick: () => void }) {
  return (
    <div className="glass rounded-2xl px-6 py-16 text-center animate-fade-in">
      <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-accent/10 border border-accent/20" style={{ animation: 'float 3s ease-in-out infinite' }}>
        <svg className="h-8 w-8 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      </div>
      <h3 className="text-lg font-semibold text-ink">Sin programaciones</h3>
      <p className="mt-1.5 text-sm text-ink-muted">Programa aportes automáticos desde tus carteras hacia tus metas.</p>
      <button type="button" onClick={onCreateClick} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-accent px-5 py-2.5 text-sm font-semibold text-[var(--bg)] shadow-lg shadow-accent/20 transition-all hover:bg-accent-light active:scale-[0.97]">
      ...
      <div className="w-full max-w-md rounded-lg border border-border bg-surface shadow-xl animate-scale-in max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          {(!bancos || bancos.length === 0) && (
            <div className="rounded-md border border-accent/20 bg-accent-subtle px-4 py-3 text-sm text-accent">
          ...
          {tipo === 'fijo' ? (
            <div>
              ...
              <input type="number" inputMode="decimal" min={1} step="0.01" value={monto || ''} onChange={(e) => setMonto(Number(e.target.value))} required placeholder="50000" className="w-full rounded-lg border border-border px-3 py-2.5 text-sm focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/20" />
            </div>
          ) : (
            <div>
