        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div className="rounded-lg border border-border bg-[#1a1e27] px-4 py-3">
            ...
            <div className="flex rounded-lg border border-border overflow-hidden">
              <button type="button" onClick={() => setTipo('personal')} className={`flex-1 px-3 py-2 text-[11px] font-semibold transition-colors ${tipo === 'personal' ? 'bg-primary text-white' : 'bg-[#141820] text-ink-muted hover:bg-[#1a1e27]'}`}>Personal</button>
              <button type="button" onClick={() => setTipo('compartida')} className={`flex-1 px-3 py-2 text-[11px] font-semibold transition-colors ${tipo === 'compartida' ? 'bg-accent text-white' : 'bg-[#141820] text-ink-muted hover:bg-[#1a1e27]'}`}>Compartida</button>
            </div>
            {tipo === 'compartida' && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" min={1} value={monto || ''} onChange={(e) => setMonto(Math.max(0, Number(e.target.value) || 0))} placeholder="0" className="w-full rounded-lg border border-border bg-[#141820] py-3 pl-9 pr-4 text-lg font-semibold placeholder:text-ink-muted focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" style={{ fontFamily: "'JetBrains Mono', monospace" }} />
            </div>
          </div>
      ...
      <div className="w-full max-w-sm rounded-lg border border-border bg-[#141820] shadow-xl animate-scale-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div>
            ...
            <div className="relative">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-base font-medium text-ink-muted">$</span>
              <input type="number" min={1} max={cartera.saldo} value={monto || ''} onChange={(e) => setMonto(Math.max(0, Math.min(cartera.saldo, Number(e.target.value) || 0)))} placeholder="0" className="w-full rounded-lg border border-border bg-[#141820] py-3 pl-9 pr-4 text-lg font-semibold placeholder:text-ink-muted focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" style={{ fontFamily: "'JetBrains Mono', monospace" }} />
            </div>
          </div>
          <div className="flex justify-end gap-3 pt-1">
            <button type="button" onClick={handleClose} className="rounded-lg border border-border px-4 py-2.5 text-sm font-medium text-ink-muted hover:bg-[#1a1e27] transition-colors">Cancelar</button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md rounded-lg border border-border bg-[#141820] shadow-xl animate-scale-in">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <button type="button" onClick={onClose} className="rounded-md p-1.5 text-ink-muted hover:bg-[#1a1e27] hover:text-ink transition-colors">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="rounded-lg border border-border px-4 py-2.5 text-sm font-medium text-ink-muted hover:bg-[#1a1e27] transition-colors">Cancelar</button>
      ...
      <div className="w-full max-w-sm rounded-lg border border-border bg-[#141820] shadow-xl animate-scale-in">
        <div className="px-6 py-5">
          ...
          <div className="mt-5 flex justify-end gap-3">
            <button type="button" onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-ink-muted hover:bg-[#1a1e27] transition-colors">Cancelar</button>
    ...
    <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 animate-fade-in">
        {!showJoinCode ? (
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="flex flex-1 items-center gap-2 rounded-lg border border-border bg-[#141820] px-3 py-2.5">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => { setShowJoinCode(false); setJoinCodeValue('') }} className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-ink-muted hover:bg-[#1a1e27] transition-colors">Cancelar</button>
      ...
      {isLoading && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-2xl border border-border bg-[#141820] p-5 space-y-3">
      ...
      {!isLoading && bancos && bancos.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 stagger">
          {bancos.map((banco) => (
            <div key={banco.id} className="rounded-2xl border border-border bg-[#141820] border-l-4 card-hover overflow-hidden" style={{ borderLeftColor: banco.color }}>
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <div className="flex gap-0.5">
                    ...
                    <button onClick={() => setEditTarget(banco)} className="rounded p-1 text-ink-muted hover:bg-[#1a1e27] transition-colors" title="Editar">
                ...
                <div className="rounded-md border border-border-light bg-[#1a1e27] px-3 py-2.5 mb-3">
