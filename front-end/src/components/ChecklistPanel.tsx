        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-black/90 backdrop-blur-sm animate-fade-in">
          <div className="flex items-center justify-between px-4 py-3 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <a href={previewUrl} download="comprobante" className="rounded-xl bg-white/10 px-4 py-2 text-sm font-medium text-white hover:bg-white/20" onClick={(e) => e.stopPropagation()}><svg className="h-4 w-4 inline mr-1.5 -mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>Descargar</a>
              <button type="button" onClick={() => setPreviewUrl(null)} className="rounded-xl p-2 text-white/70 hover:text-white hover:bg-white/10"><svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
          </div>
          <div className="flex-1 flex items-center justify-center p-4">
            {previewUrl.startsWith('data:image/') ? <img src={previewUrl} alt="Comprobante" className="max-w-full max-h-full object-contain rounded-xl" /> : <iframe src={previewUrl} className="w-full h-full rounded-xl bg-white" title="Comprobante" />}
          </div>
        </div>, document.body)}
    </div>
  )
