      ...
      <div className="flex flex-col lg:flex-row w-full max-w-5xl items-center justify-center lg:gap-8">
        <div className="hidden lg:flex lg:flex-1 flex-col justify-center px-6 py-12 lg:px-16 lg:py-0">
          <div className="max-w-lg mx-auto lg:mx-0 animate-slide-up">
            ...
            <p className="text-[11px] text-ink-muted">Empezá a gestionar tus finanzas de forma inteligente.</p>
          </div>
        </div>
        <div className="flex items-center justify-center w-full lg:w-[450px]">
          <div className="w-full max-w-sm animate-slide-up flex flex-col gap-5">
            ...
            <div className="rounded-2xl p-6 sm:p-8 bg-surface border border-border shadow-xl">
              <div className="mb-6"><h2 className="text-lg font-extrabold text-ink">{isRegister ? 'Creá tu cuenta' : 'Iniciá sesión'}</h2><p className="text-xs text-ink-muted mt-1">{isRegister ? 'Empezá a gestionar tus finanzas.' : 'Continuá donde lo dejaste.'}</p></div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={submitting || !email || password.length < 6 || (isRegister && !name)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3.5 text-sm font-semibold text-white shadow-lg shadow-primary/20 transition-all hover:bg-[#7C3AED] active:scale-[0.98] disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-primary/30">
                  {submitting ? <div className="flex items-center gap-2"><div className="h-5 w-5 animate-spin rounded-full border-2 border-white/30 border-t-white" />{isRegister ? 'Creando cuenta...' : 'Ingresando...'}</div> : isRegister ? 'Crear cuenta' : 'Iniciar sesión'}
                </button>
              </form>
              ...
              <button type="button" onClick={handleGoogleLogin} disabled={submitting} className="flex w-full items-center justify-center gap-3 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-primary/20 transition-all hover:bg-[#7C3AED] active:scale-[0.98] disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-primary/30">
                {submitting ? <><div className="h-5 w-5 animate-spin rounded-full border-2 border-white/30 border-t-white" />Conectando...</> : <><svg className="h-5 w-5" viewBox="0 0 24 24"><path fill="#fff" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" /><path fill="#fff" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="#fff" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" /><path fill="#fff" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" /></svg>Continuar con Google</>}
              </button>
              <p className="text-center text-xs text-ink-muted mt-5">{isRegister ? <span>¿Ya tenés cuenta? <button type="button" onClick={() => setIsRegister(false)} className="font-semibold text-primary hover:underline">Entrar</button></span> : <span>¿Primera vez? <button type="button" onClick={() => setIsRegister(true)} className="font-semibold text-primary hover:underline">Crear cuenta</button></span>}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[var(--bg)]">
      <div className="flex flex-col items-center gap-6 animate-fade-in">
        ...
        <div className="flex flex-col items-center gap-1.5">
          <p className="text-base font-semibold text-shimmer tracking-wider">Cargando...</p>
    ...
    <div className="flex min-h-screen bg-[var(--bg)]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-60 flex-col border-r border-border bg-[var(--bg-sidebar)] transition-transform duration-300 lg:static lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          ...
          <NavLink to="/bancos" className={navLinkClass} onClick={() => setSidebarOpen(false)}><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>Bancos</NavLink>
          <NavLink to="/tarjetas" className={navLinkClass} onClick={() => setSidebarOpen(false)}><svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" /></svg>Tarjetas</NavLink>
