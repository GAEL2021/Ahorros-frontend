...
export class PresupuestosController {
  ...
  @Post(':id/gastos/:gastoId/pagar') async pagarGasto(@Param('id') id: string, @Param('gastoId') gastoId: string, @Body() dto: PagarGastoDto, @Req() req: Request) { try { return await this.service.pagarGasto(id, gastoId, req.user as FirebaseUser, dto.montoReal, dto.carteraId, dto.medioDePago, dto.tarjetaCreditoId, dto.fechaPago); } catch (e: any) { throw new BadRequestException(e.message); } }
